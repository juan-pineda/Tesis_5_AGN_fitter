...
def CATALOG_settings():
    ...
    ***USER INPUT NEEDED***
    ...
    cat['filename'] = cat['path']+'brown_test/brown2018_syn_IR-UV_2.txt'   # -2
   ...
   ##FREQUENCIES/WAVELENGTHS 
    ...
    cat['freq/wl_list'] = np.arange(2,126,3).tolist() #np.arange(5,48,2).tolist()                                
    ...
    cat['flux_list'] = np.arange(3,127,3).tolist()       
                                        ## If ASCII: List of column indexes (int)
    #cat['flux_suffix'] = '_f'          ## If FITS: Common ending of all flux column names (str)    
    cat['fluxerr_list'] = np.arange(4,128,3).tolist() 
                                        ## If ASCII: List of column indexes (int)
...
def FILTERS_settings():
    ...
    filters['dict_zarray'] = np.array([0.058900, 0.042170]) #  np.array([0.283, 1.58])  # Deprecated. The grid of redshifts needed to fit your catalog
    filters['path'] = 'models/FILTERS/' 
    filters['filterset'] = 'example_42datapointa' ## 'filterset_default' (for the test case),
...
def MODELS_settings():
    ...
    models['TORUS'] ='SKIRTORM' ### Current options:
                           ...
                           ### 'SKIRTORM_3P' SKIRTOR model with averaged SEDs for each inclination, openning angle and optical depth
                           ### 'SKIRTORM_4P' SKIRTOR model with averaged SEDs for each inclination, openning angle, optical depth and p
